...
@mark.gate
def test_ft_vlan_removed_from_end_of_table(topology, step):
    ...
    step("Step 8- Delete highest vlan from table")
    with ops1.libs.vtysh.Configure() as ctx:
        ctx.no_vlan(4)

    step("Step 9- Verify vlan was deleted")
    assert findvlan(ops1, "4") == 0
    ...
    assert findport_vlan(ops1, '2', '9') == 1
